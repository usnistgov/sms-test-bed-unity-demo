...
public static class Poly2Mesh
{
    ...
    public static Mesh CreateMesh(Polygon polygon)
    {
        ...
        foreach (DelaunayTriangle t in poly.Triangles)
        {
            foreach (var p in t.Points)
            {
                ...
                if (!codeToPosition.TryGetValue(p.VertexCode, out Vector3 pos))
                {
    ...
    public static GameObject CreateGameObject(Polygon polygon, bool workshop, string name = "Polygon")
    {
        GameObject gob = new GameObject { name = name };
        ...
        gob.AddComponent(typeof(MeshCollider));
        gob.GetComponent<MeshCollider>().sharedMesh = CreateMesh(polygon);
        ...
        { 
            ...
            Type type = asm.GetType("OnClickMachineInfo");
            gob.AddComponent(type);
        }
